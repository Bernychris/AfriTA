...
# Style CSS pour la barre d'en-tête dorée
st.markdown("""
<style>
/* Barre d'en-tête supérieure (où se trouvent >> et ...) */
header {
    background: linear-gradient(90deg, #0a0a0f 0%, #1a1a2a 100%) !important;
    border-bottom: 2px solid #ffd700 !important;
    box-shadow: 0 2px 10px rgba(255, 215, 0, 0.2) !important;
}

/* Icônes (flèches >> et trois points ...) */
button[kind="header"] svg {
    color: #ffd700 !important;
    fill: #ffd700 !important;
}

/* Au survol des icônes */
button[kind="header"]:hover {
    background: rgba(255, 215, 0, 0.15) !important;
    border-radius: 50% !important;
}

/* Menu déroulant des trois points */
div[data-testid="stHeader"] {
    background: #0f0f0f !important;
}

/* Texte dans la barre d'en-tête si présent */
header * {
    color: #ffd700 !important;
}
</style>
...
# Menu personnalisé en haut (SANS "Voir fact")
st.markdown("""
...
# Pages (avec "Voir fact" dans la sidebar seulement)
accueil = st.Page("pages/accueil.py", title="🏠 Accueil", icon="🏠")
panier = st.Page("pages/panier.py", title="🛒 Panier", icon="🛒")
recherche = st.Page("pages/recherche_facture.py", title="📄 Voir fact", icon="📄")
...
pg = st.navigation([accueil, panier, recherche, contact, aide, admin])
pg.run()
