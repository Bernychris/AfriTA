...
.stButton > button:hover { transform: scale(1.02); background: linear-gradient(90deg, #ff9d1a, #ffc467); }
</style>
...
with col1:
    ...
    if st.button("Choisir", key="chien", use_container_width=True):
        st.switch_page("pages/chien.py")

with col2:
    st.markdown('<div class="animal-card"><div class="animal-emoji">🐈</div><h3>Chat</h3></div>', unsafe_allow_html=True)
    if st.button("Choisir", key="chat", use_container_width=True):
        st.switch_page("pages/chat.py")

with col3:
    st.markdown('<div class="animal-card"><div class="animal-emoji">🐇</div><h3>Lapin</h3></div>', unsafe_allow_html=True)
    if st.button("Choisir", key="lapin", use_container_width=True):
        st.switch_page("pages/lapin.py")

with col4:
    st.markdown('<div class="animal-card"><div class="animal-emoji">🐦</div><h3>Oiseau</h3></div>', unsafe_allow_html=True)
    if st.button("Choisir", key="oiseau", use_container_width=True):
        st.switch_page("pages/oiseau.py")
