...
# BANDEAU BLEU
st.markdown("""
<div style="background: linear-gradient(90deg, #2196f3, #1976d2); padding: 0.5rem; border-radius: 10px; margin-bottom: 1rem;">
    <p style="color: white; text-align: center; margin: 0; font-weight: bold;">🔵 SERVICE CHIEN - TRANSPORT CANIN</p>
</div>
...
if distance is not None:
    ...
    if st.button("✅ Générer ma facture", use_container_width=True, key="chien_generer"):
        if nom_client and telephone:
            ...
            # Message pour enregistrer le numéro
            st.info(f"""
...
- Vous en aurez besoin pour voir votre facture
...
""")
            ...
            # Bouton pour copier
            st.markdown(f"""
            <button onclick="navigator.clipboard.writeText('{numero_facture}')" style="background: #2d8c3e; color: white; border: none; border-radius: 50px; padding: 0.5rem 1rem; cursor: pointer; margin-top: 0.5rem;">
                📋 Copier le numéro
            </button>
            """, unsafe_allow_html=True)
