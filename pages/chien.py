...
with col3:
    # Race avec option "Autre"
    liste_races = list(races.keys()) + ["✏️ Autre race (précisez)"]
    race_choisie = st.selectbox("Race", liste_races, key="chien_race")
    
    # Si "Autre race" est sélectionné, afficher un champ texte
    if race_choisie == "✏️ Autre race (précisez)":
        autre_race = st.text_input("Précisez la race de votre chien", placeholder="Ex: Berger Australien, Yorkshire, etc.", key="chien_autre_race")
        # Demander la catégorie de race
        categorie_race = st.radio("Catégorie de la race", ["Standard (prix normal)", "Premium (+30%)", "Très rare (+60%)"], key="chien_categorie")
...
with col4:
    pays_depart = st.selectbox("Pays de départ", ["Cameroun", "Gabon", "Congo", "Tchad", "Nigéria"], key="chien_depart")
    pays_arrivee = st.selectbox("Pays d'arrivée", ["Cameroun", "Gabon", "Congo", "Tchad", "Nigéria"], key="chien_arrivee")
...
if st.button("✅ Générer ma facture", use_container_width=True, key="chien_generer"):
    if nom_client and telephone:
        ...
        nouvelle_facture = pd.DataFrame([{"Numero": numero_facture, "Date": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "Client": nom_client, "Telephone": telephone, "Email": email, "Animal": "Chien", "Race": race_affichee, "Coeff": coeff_race, "Poids": poids, "Depart": pays_depart, "Arrivee": pays_arrivee, "Distance": distance, "Prix": int(prix), "Statut": "En attente paiement"}])
