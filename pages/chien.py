...
.warning-box { background: #1e1e1e; padding: 1rem; border-radius: 10px; border-left: 4px solid #ff8c00; margin: 1rem 0; }
</style>
...
if distance is not None:
    ...
    st.markdown(f'<div class="info-box">📏 Distance : <strong>{distance} km</strong><br>📍 {zone}<br>⭐ Coefficient race : <strong>x{coeff_race}</strong></div>', unsafe_allow_html=True)
    ...
    if st.button("✅ Générer ma facture", use_container_width=True, key="chien_generer"):
        if nom_client and telephone:
            ...
            if not os.path.exists(data_file):
                pd.DataFrame(columns=["Numero", "Date", "Client", "Telephone", "Email", "Animal", "Race", "Poids", "Depart", "Arrivee", "Distance", "Zone", "Prix", "Statut"]).to_csv(data_file, index=False)
            nouvelle_facture = pd.DataFrame([{"Numero": numero_facture, "Date": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "Client": nom_client, "Telephone": telephone, "Email": email, "Animal": "Chien", "Race": race_affichee, "Poids": poids, "Depart": pays_depart, "Arrivee": pays_arrivee, "Distance": distance, "Zone": zone, "Prix": int(prix), "Statut": "En attente paiement"}])
    ...
    <div class="warning-box">
        🌍 <strong>Distance non disponible</strong><br><br>
        Veuillez nous contacter pour ce trajet :<br>
        📞 +237 6XX XXX XXX | 📧 contact@afrita-transport.com
    </div>
    """, unsafe_allow_html=True)
