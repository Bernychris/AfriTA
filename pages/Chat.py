...
.stButton > button:hover { transform: scale(1.02); background: linear-gradient(90deg, #ab47bc, #9c27b0); }
...
with col3:
    ...
    poids = st.number_input("Poids (kg)", min_value=1, max_value=100, value=5)
with col4:
...
if st.button("✅ Générer ma facture", use_container_width=True):
    if nom_client and telephone:
        ...
        nouvelle_facture = pd.DataFrame([{"Numero": numero_facture, "Date": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "Client": nom_client, "Telephone": telephone, "Animal": "Chat", "Race": race_choisie, "Poids": poids, "Depart": pays_depart, "Arrivee": pays_arrivee, "Prix": int(prix)}])
