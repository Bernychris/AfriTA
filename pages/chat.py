...
with col3:
    ...
    if race_choisie == "✏️ Autre race (précisez)":
        autre_race = st.text_input("Précisez la race", placeholder="Ex: Sacré de Birmanie", key="chat_autre_race")
        categorie_race = st.radio("Catégorie", ["Standard", "Premium (+30%)", "Très rare (+60%)"], key="chat_categorie")
...
with col4:
    pays_depart = st.selectbox("Pays de départ", pays_afrique, key="chat_depart")
    pays_arrivee = st.selectbox("Pays d'arrivée", pays_afrique, key="chat_arrivee")
...
if distance is not None:
    ...
    if st.button("✅ Générer ma facture", use_container_width=True, key="chat_generer"):
        if nom_client and telephone:
            ...
            nouvelle_facture = pd.DataFrame([{"Numero": numero_facture, "Date": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "Client": nom_client, "Telephone": telephone, "Email": email, "Animal": "Chat", "Race": race_affichee, "Poids": poids, "Depart": pays_depart, "Arrivee": pays_arrivee, "Distance": distance, "Prix": int(prix), "Statut": "En attente paiement"}])
